...
class ValueAveraging(Strategy): 
    ...
    def init(self):
        ...
        self.average = True         # 是否使用價值平均
        self.canSell = True         # 是否要將盈餘取出
    ...
    def next(self):
        self.dayCont += 1       
        if self.dayCont % self.dayContcycle == self.dayMod:    # 當到了定期購買週期
            self.mounthCont += 1

            print("第次交易 :", self.mounthCont, "股票數量 :", self.sumShares)

            if self.average:        # 是否使用價值平均
                ...
                self.buySharessize = self.mounthCost // self.data.Close[-1]         # 計算價值 mounthCost 金額等價股數
                if self.buySharessize >= 1:                                         # 確保不為0
                    self.buy(size = self.buySharessize)                             # 買入不股數
                    self.sumShares += self.buySharessize
...
if __name__ == '__main__':
    ...
    test = Backtest(df, ValueAveraging, cash=490000, commission=.004)
