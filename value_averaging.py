    ...
    def next(self):
        ...
        if self.dayCont > self.dayContcycle:    # 當到了定期購買週期
            ...
            if self.actualValue > self.expectedValue :      # 如果實際價值大於預計價值
                sellSharessize = (self.actualValue - self.expectedValue) // self.data.Close[-1]
                self.sell(size = sellSharessize)            # 賣出多餘股數
                self.cashSurplus = sellSharessize * self.data.Close[-1]
                self.sumShares -= sellSharessize

            elif self.actualValue < self.expectedValue :
                buySharessize = (self.expectedValue - self.actualValue) // self.data.Close[-1]
                self.buy(size = buySharessize)              # 買入不足股數
                self.sumShares += buySharessize

            elif self.expectedValue ==  0:                  # 第一次購入
                buySharessize = self.mounthCost // self.data.Close[-1]
                print(buySharessize)
                self.buy(size = buySharessize)              # 買入第一次股數
                self.sumShares += buySharessize

            print("股票價值 :", round(self.sumShares * self.data.Close[-1]), "現金盈餘 :", round(self.cashSurplus))
            print(self.mounthCont)
...
if __name__ == '__main__':
    ...
    st_tool = StockTool(target_stock='0050.tw', date_range = "20200101-20211223")
    df = st_tool.crawler2pandas()
    # st_tool.crawler2CSV()

    test = Backtest(df, ValueAveraging, cash=75000, commission=.004)
